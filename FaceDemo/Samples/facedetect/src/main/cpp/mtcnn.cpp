...
MTCNN::MTCNN(){
#ifdef LINUX
	#ifdef ARM64_DEBUG
		printf("structure funcion \n");
	#endif
#endif

#ifdef LINUX
		std::string home = getenv("HOME");
		std::string jlx_home =  home + "/Jlx";
		std::string mtcnn_home = jlx_home + "/mtcnn";

		std::string model_path =  mtcnn_home + "/models";
...
#ifdef LINUX
	#ifdef ARM64_DEBUG
		printf("mtcnn  structure funcion      model_path  : %s\n",model_path.c_str());
	#endif
#endif


#ifdef ANDROID
	LOG(INFO) << "param_files.size():    " << param_files.size() << std::endl;
	LOG(INFO) <<"bin_files.size():    "         << bin_files.size()  << std::endl;
#else
	std::cout << "param_files.size():    " << param_files.size() << std::endl;
	std::cout << "bin_files.size():    " << bin_files.size()  << std::endl;
#endif

	for(int i = 0; i< param_files.size();i++){
		if(0 != access(param_files.at(i).c_str(),F_OK)){
			#ifdef ANDROID
			LOG(ERROR) << "Not find file: " << param_files.at(i).c_str()<< std::endl;
			#else
			std::cout  << "Not find file: " << param_files.at(i).c_str() << std::endl;
			#endif
		}
	}
	for(int i = 0; i< bin_files.size();i++){
		if(0 != access(bin_files.at(i).c_str(),F_OK)){
			#ifdef ANDROID
			LOG(ERROR) << "Not find file: " << bin_files.at(i).c_str() << std::endl;
			#else
			std::cout << "Not find file: " << bin_files.at(i).c_str() << std::endl;
			#endif
		}
	}
...
MTCNN::MTCNN(const string &model_path) {
	...
	for(int i = 0; i< param_files.size();i++){
		if(0 != access(param_files.at(i).c_str(),F_OK)){
			...
			#else
			std::cout  << "Not find file: " << param_files.at(i).c_str();
			#endif
		}
	}
	for(int i = 0; i< bin_files.size();i++){
		if(0 != access(bin_files.at(i).c_str(),F_OK)){
			...
			#else
			std::cout << "Not find file: " << bin_files.at(i).c_str();
			#endif
		}
	}
...
#ifdef LINUX
	#ifdef ANDROID
		LOG(INFO) << " SetMinFace " << minSize << std::endl;
	#else
		printf(" SetMinFace %d\n",minSize);
	#endif
#endif
...
#ifdef LINUX
	#ifdef ANDROID
		LOG(INFO) << " SetNumThreads " << numThreads << std::endl;
	#else
...
extern "C"{
    ...
    bool c_setMinface(MTCNN* m,int minsize){
				if( m != NULL){
    ...
    bool c_setNumber(MTCNN* m,int numThreads){
				if( m != NULL){
